//Se agrega funcionalidad de beacon
...
#define DEVICE_NAME                     "Nordic_UARTER"                               /**< Name of device. Will be included in the advertising data. */
...
#define APP_MINOR_VALUE                 0x00, 0xFF                        /**< Minor value used to identify Beacons. */ 
#define APP_BEACON_UUID                 0x01, 0x12, 0x23, 0x34, \
                                        0x45, 0x56, 0x67, 0x78, \
                                        0x89, 0x9a, 0xab, 0xbc, \
                                        0xcd, 0xde, 0xef, 0xAB            /**< Proprietary UUID for Beacon. */
...
static void advertising_init2(void)
{
    ...
    ble_advdata_manuf_data_t manuf_specific_data;
		...
		ble_advdata_manuf_data_t        manuf_data; // Variable to hold manufacturer specific data //ojo es la manual
		uint8_t data[]                      = "@@@@@@"; // Our data to adverise
...
//#endif
    ...
    advdata.p_manuf_specific_data = &manuf_specific_data;
		//advdata.p_manuf_specific_data = &manuf_data;//ojo
...
int main(void)
{
    ...
    services_init();
    //advertising_init();
		advertising_init2();//ojo
    //conn_params_init();
    adc_init();
    printf("%s",start_string);

    //err_code = ble_advertising_start(BLE_ADV_MODE_FAST);
		advertising_start();//ojo agregado
    APP_ERROR_CHECK(err_code);
		adc_start();
		for (int i=0;i<APP_BEACON_INFO_LENGTH;i++){
			printf("%02X",m_beacon_info[i]);
		}
